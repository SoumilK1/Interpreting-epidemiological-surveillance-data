...
//    val publicTransport = if (biasedCoinToss(PUBLIC_TRANSPORT_FRACTION)) 1 else 0
    ...
    val isEssentialWorker = if (biasedCoinToss(ESSENTIAL_WORKER_FRACTION)) 1 else 0
    val officeId = if (!isEssentialWorker) random.between(1, totalOffices + 1) else 0
